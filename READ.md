...
**GSE149689의 Healthy 1개와 severe COVID-19 1개**, 총 두 사람의 PBMC를 사용합니다. 두 sample 모두 **10x Chromium Single Cell 3′ v3의 droplet 방식**으로 만들어졌습니다. 강의에서 배운 cell barcode와 UMI를 실제 데이터 분석에 연결합니다.
...
Sample 선정과 플랫폼은 GEO 정보를 기준으로 합니다. **그룹당 한 명이므로 질환 효과와 개인·성별·기술적 차이를 분리할 수 없습니다.** 수업에서는 두 sample에서 관찰한 패턴을 비교하며, 환자 전체에 대한 통계적 결론을 내리지 않습니다.

### 학생용 파일 준비

아래 두 파일을 내려받아 폴더에 둡니다. GitHub 파일 화면에서는 **Download raw file**을 선택합니다. 학생은 원본 전체 matrix를 받을 필요가 없습니다. 다운로드 파일이 제공되지 않은 복사본을 사용하는 경우 마지막 **강사용 데이터 준비** 절로 같은 파일을 만들 수 있습니다.
...
IFN1은 control gene과 비교한 상대 점수입니다. Monocyte 안에서도 CD14/CD16 subtype 구성에 따라 값이 달라질 수 있습니다. 두 사람만으로 질환 효과를 확정하지 않으며, 이번 실습에서는 질환군 간 p-value를 계산하지 않습니다. 이 두 sample에서 차이가 뚜렷할지는 실제 결과를 보고 판단합니다.
...
## 강사용 데이터 준비 — 학생 실습 전에 한 번 실행

학생에게는 아래에서 만드는 `H1_counts.rds`, `COVID1_counts.rds` 두 파일을 배포합니다. 파일이 이미 제공된 학생은 이 절을 실행하지 않습니다. 이 README의 1절에서 Seurat를 설치한 환경이 필요합니다.

### A. GEO 원본 다운로드

[GSE149689](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE149689)의 Supplementary file 세 개를 사용합니다. 전체 matrix 파일은 압축 상태로 약 467 MB이며, 읽을 때는 더 많은 RAM이 필요합니다. `.gz`는 풀지 않습니다. 기존 결과가 있는 폴더에서는 같은 이름의 파일을 덮어쓸 수 있습니다.

```r
dir.create("data/GSE149689_original", recursive = TRUE, showWarnings = FALSE)
options(timeout = 1800)
geo_url <- "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE149nnn/GSE149689/suppl/"

download.file(paste0(geo_url, "GSE149689_matrix.mtx.gz"),
              "data/GSE149689_original/GSE149689_matrix.mtx.gz", mode = "wb")
download.file(paste0(geo_url, "GSE149689_features.tsv.gz"),
              "data/GSE149689_original/GSE149689_features.tsv.gz", mode = "wb")
download.file(paste0(geo_url, "GSE149689_barcodes.tsv.gz"),
              "data/GSE149689_original/GSE149689_barcodes.tsv.gz", mode = "wb")
```

큰 파일의 다운로드를 위해 대기 시간을 30분으로 늘렸습니다. 실패하면 네트워크와 다운로드된 파일을 확인하고 해당 다운로드 줄을 다시 실행합니다.

### B. Matrix 읽기와 sample 선택

접두어가 붙은 원본 파일명을 그대로 사용하기 위해 `ReadMtx()`에 파일 세 개를 지정합니다. GEO에서는 여러 sample의 count를 `cellranger aggr --normalize=mapped`로 합쳤다고 설명하므로, 이는 제출자가 제공한 aggregated count입니다.

```r
all_counts <- Seurat::ReadMtx(
  mtx = "data/GSE149689_original/GSE149689_matrix.mtx.gz",
  cells = "data/GSE149689_original/GSE149689_barcodes.tsv.gz",
  features = "data/GSE149689_original/GSE149689_features.tsv.gz"
)

H1_counts <- all_counts[, grepl("-5$", colnames(all_counts))]
COVID1_counts <- all_counts[, grepl("-1$", colnames(all_counts))]
c(H1 = ncol(H1_counts), COVID1 = ncol(COVID1_counts))
```

`-5$`는 barcode가 정확히 `-5`로 끝난다는 뜻입니다. `-1$`은 Sample 1만 선택하며 Sample 10–19를 포함하지 않습니다. **두 값이 모두 0보다 큰지** 확인합니다. 이 단계에서 세포 QC나 무작위 추출은 하지 않습니다.

### C. 학생용 시작 파일 저장

```r
dir.create("data/GSE149689_2sample", recursive = TRUE, showWarnings = FALSE)
saveRDS(H1_counts, "data/GSE149689_2sample/H1_counts.rds", compress = "xz")
saveRDS(COVID1_counts, "data/GSE149689_2sample/COVID1_counts.rds", compress = "xz")
rm(all_counts, H1_counts, COVID1_counts)
gc()
```

두 파일을 README와 함께 `data/GSE149689_2sample/` 경로로 GitHub에 올리면 문서 처음의 상대경로 링크가 연결됩니다. 학생은 파일을 받은 뒤 1–14절을 진행합니다. 원본 전체 matrix와 검증용 중간 object는 배포하지 않아도 됩니다.

### 수업 전 확인할 결과

- H1/COVID1이 각각 Healthy/Severe_COVID로 지정되는지 확인합니다.
- QC 분포와 `qc_summary`를 보고 필터를 조정하고, 두 sample 모두 충분한 cell이 남는지 확인합니다.
- 강사와 학생의 package 버전 및 downsampling 선택을 맞춥니다.
- Cluster 번호에 고정 정답을 붙이지 말고 marker를 보고 annotation합니다.
- Monocyte IFN-response 차이는 실제 그래프에서 확인합니다. 선택한 두 사람에서 선명한 차이가 보인다고 미리 단정하지 않습니다.
- Sample과 질환 조건이 완전히 겹칩니다. Integration은 이를 해결하지 않으며, 통합 전후 결과와 원래 RNA 발현을 함께 확인합니다.
